...
public class F1_Section2_ShadowTransformer implements ViewPager.OnPageChangeListener, ViewPager.PageTransformer {
    ...
    public F1_Section2_ShadowTransformer(ViewPager viewPager, CardAdapter adapter) {
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        if (currentCard != null) {
            if (scalingEnabled) {
                currentCard.setScaleX((float) (1 + 0.13 * (1 - realOffset)));
                currentCard.setScaleY((float) (1 + 0.13 * (1 - realOffset)));
            }
            currentCard.setCardElevation(baseElevation/2);
        }
        ...
        if (nextCard != null) {
            if (scalingEnabled) {
                nextCard.setScaleX((float) (1 + 0.13 * (realOffset)));
                nextCard.setScaleY((float) (1 + 0.13 * (realOffset)));
            }
            nextCard.setCardElevation(baseElevation/2);
        }
